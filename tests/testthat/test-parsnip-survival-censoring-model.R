...
test_that("`reverse_km()`: fit reverse Kaplan-Meier curves", {
  ...
  expect_identical(mod_fit$censor_probs$label, "reverse_km")
  expect_identical(mod_fit$censor_probs$required_pkgs, "prodlim")
  ...
  expect_identical(rev_km_fit, exp_rev_km_fit)
})
...
test_that("predict with reverse Kaplan-Meier curves", {
  ...
  expect_identical(nrow(pred_df), length(pred_times))
  expect_identical(pred_df[[1]], exp_pred)

  pred_vec <- predict(mod_fit$censor_probs, time = pred_times, as_vector = TRUE)
  expect_type(pred_vec, "double")
  expect_identical(pred_vec, exp_pred)
})
